...
def is_complete_job(job, required_keys):
    return all(key in job and job[key] for key in required_keys)

def is_duplicate_job(job_url, seen_urls):
    return job_url in seen_urls
...
def save_jobs_to_csv(jobs: list, filename: str):
    ...
    fieldnames = ["title", "company", "location", "summary", "date", "job_url"]
    with open(filename, mode="w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
